...
import java.util.concurrent.{ConcurrentHashMap => JConcurrentMap}
...
class Evaluator(evals: Array[ExprNodeEvaluator]) extends IEvaluator {
  ...
  val delegate: IEvaluatorDelegate = {
    Evaluator.map.getOrElseUpdate(evals.length, {
...
object Evaluator {
  ...
  val map:mutable.ConcurrentMap[Int, IEvaluatorDelegate] = new JConcurrentMap[Int, IEvaluatorDelegate]()
